...
(function() {
	...
	function bounsscore() {
		...
		gotoDetailProfitability = function(profitabilityType) {
			...
			aladdin.navigator.forward({
				title: '交易明细',
				url: '../salespoints/financingDetails.html?rowKey='+rowKey+'&financingType='+CPML_NAME06,
				// 默认情况下即是`webapp`，可以不传
				type: 'webapp',
				...
				success: function(err, res) {
					...
					if(listInfs.length > -1)
						...
						for(var i = 0; i < listInfs.length; i++) {
							...
							$("#right_table2").append("<tr><td><div>" + listMap.CPML_NAME06 + "</div></td><td><div>" + chuangli + "</div></td><td><div>" + listMap.MONTH_ZS_ZZ + "</div></td><td><div>" + bili + "</div></td><td><div>" + listMap.BUY_DATE + "</div></td><td><div>" + listMap.MATURITY_DATE + "</div></td><td><div>" + listMap.TRAN_AMT + "</div></td><td><div>" + listMap.CL_TYPE + "</div></td><td><div>" + listMap.SALES_SY_RATE_ZZ + "</div></td><td><div>" + listMap.PROD_GUIDE_XS + "</div></td><td><div>" + zizhi + "</div></td><td><div>" + listMap.BECIF_NO + "</div></td><td><div>" + listMap.CARD_NO + "</div></td></tr>");
	...
	//							if(Math.abs(endPos.y) > 10){	
				...
				//financingInfs("1212","121",datetime,page);
